...
#PROGRAM PROMPT: This is a temperature conversion program, it allows a user to enter as 
#many Fahrenheit temps as they'd like and then shows the Celsius conversoion for each. It 
#also counts the number of temps and determines the average of all temps entered. 
...
def again(): #<---Function Header
    '''this function ask a user if they'd like to enter another temp, 
    checks the response for validity, and then returns a valid response back to the main program'''
...
def converter(f):
    #'f' is parameter
    '''this fucntion is passed an argument value of 'f' (a tempF), converts to tempC, and then returns the conversion value'''
    c = (f - 32) * (5 / 9)
